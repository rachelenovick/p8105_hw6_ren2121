...
I propose that baby birth weight is based on the following variables:
gestational age, maternal characteristics (including age, height,
pre-pregnancy BMI, weight gain during pregnancy, and smoking), and baby
sex. I chose these predictors because they are factors known to
influence birth weight based on biological plausibility and prior
research. I included these variables as continuous predictors other than
baby sex, which was included as a two-level factor variable.

I decided to include these variables which are commonly associated with
baby birth weight, excluding other potential predictor variables to
avoid overfitting the model and keep the model interpretable.
...
![](p8105_hw6_ren2121_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

Most of the data points seem to lie between approximately 2500g and
3500g, and residuals seems to mostly lie between -1000g and 1000g. There
does not seem to be a major problem with heteroskedasticity based on the
...
![](p8105_hw6_ren2121_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->

Comparing root mean squared error between the three models, I found that
the model I created with gestational age, maternal characteristics, and
baby sex as predictors of baby birth weight performed worse than the
other two models, as indicated by a higher distribution of higher RMSE
values. The model with head circumference, length, sex and all
interactions had the best predictive performance, with the highest
proportion of lower RMSE values.
